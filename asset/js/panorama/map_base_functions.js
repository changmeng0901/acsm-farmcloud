...
function drawingPolygon(pointArr, polygonColor, theme, isEditable,marker) {
	var triangleCoords = [];
...
function getBoundsZoomLevel(bounds) {
	var mapDim = {
		    height: jQuery("#map-canvas").height(),
		    width: jQuery("#map-canvas").width()
		};
...
CustomOverlay.prototype.polygonMoved = function(newLatLng)
{
    ...
    if(base!=null){
		_Q("#krpanos_base_id").val(base._id);		
	}else{
		//alert("请将四季田景放到基地范围内!");
		_Q("#krpanos_base_id").val("");
	}
    jQuery("#krpanos_coordinate_X").val(lat);
	jQuery("#krpanos_coordinate_Y").val(lng);
};
//设备可以移动
function move(tempThis,shape){ 	
	jQuery(shape).draggable({
        ...
        stop:function(event,ui){
        	krpanoObj = tempThis;
        	var width = jQuery(tempThis.div).find("img").width();
        	var height = jQuery(tempThis.div).find("img").height();
