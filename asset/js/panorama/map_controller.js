...
var assetRoot="";//接口IP
var ajaxRoot="";//接口地址
var googleMap_center = [];//地图中心点
var krpanos_flag=false;//是否有四季田景标识

//加载地图
function loadZyMap(domain_url,coordinate_X,coordinate_Y,flag){
	assetRoot=domain_url;
	ajaxRoot=assetRoot+'/rest/1.0/service?v=1.0&format=json&sign=5C21760DB93BB7B1A2CD0A9BE6B1E67E';	
	googleMap_center.push(coordinate_X);
	googleMap_center.push(coordinate_Y);
	gMapInit(document.getElementById("map-canvas"), googleMap_center);	
	krpanos_flag=flag;
	//如果田景没有坐标，则根据鼠标点击的位置显示
...
//加载基地信息
function loadZyBase(farme_baseID){
...
//四季田景
function loadKrpanos(krpano_coordinate_X,krpano_coordinate_Y){	
	var arr = {};
	arr.device_ = 4;
	var latlng ;
	if(krpano_coordinate_X != "" && krpano_coordinate_Y != ""){
		var latlng = makeLatLng([krpano_coordinate_X, krpano_coordinate_Y]);
		var custom = new CustomOverlay(map, "", latlng, arr);
		latlng = new google.maps.LatLng(parseFloat(krpano_coordinate_X),parseFloat(krpano_coordinate_Y));
		map.setCenter(latlng);
		krpanos_flag=true;
		jQuery("#krpanos_coordinate_X").val(krpano_coordinate_X);
		jQuery("#krpanos_coordinate_Y").val(krpano_coordinate_Y);
	}	
}

function setMapCenter(coordinate_X,coordinate_Y){
	latlng = new google.maps.LatLng(parseFloat(coordinate_X),parseFloat(coordinate_Y));
	map.setCenter(latlng);
}
...
function loadC1(markstr) {
	...
	for (var i = 0; i < markstr.length; i++) {
		...
		drawingPolygon(k1Arr, markstr[i]['color'], 1,false,markstr[i]);
...
function clearPop() {
	...
	closeVideo3();
	closeVideo4();
	if (jQuery("#map_loading").is(":hidden")) {
		if (jQuery("#farmeImg").length > 0) {
			jQuery("#farmeImg").remove();
		}
		if (jQuery("#krpano_close_btn").length > 0) {
			jQuery("#krpano_close_btn").remove();
		}
		jQuery(".mengb").hide();
		setTimeout(function() {
			jQuery("#farmInfoD").hide();
			jQuery("#xiugeng").hide();
			jQuery("#noCrops").hide();
			jQuery("#sensor").hide();
			jQuery("#weatherStation").hide();
		}, 200);
	}
}
//农场信息弹出
function farmeInfoShow(obj) {
	...
	jQuery("#farmInfoD h2").html(parentData.name);
	jQuery("#farmInfoD .farmpic img").attr("src", parentData.baseImageUrl); //之前是baseImageUrl
	jQuery("#farmInfoD .fl_concat").html(parentData.contact);
	jQuery("#farmInfoD .phone").html(parentData.phone);
	jQuery("#farmInfoD .cm_address").html(parentData.address);
	jQuery("#farmInfoD .farminfo_footer").html('描述：' + parentData.description);
}
//
function showInfoWindow(obj) {
	//(1)jQuery(".mengb").show();
	//(2)jQuery("#map_loading").show();
...
function setPopOutDivPosition(id) {
	var infodW = jQuery(id).width();
	var infodH = jQuery(id).height();
	var DW = jQuery(".map_content").width();
	var DH = jQuery(".map_content").height();
	if (infodH * 2 + 30 <= DH) {
		jQuery(id).css({
			"left": (DW - infodW) / 2 - 3,
			"top": DH / 2 - infodH - 30
		}).show();
		jQuery(id).children(".supIco").show();
	} else {
		jQuery(id).css({
			"left": (DW - infodW) / 2 - 3,
			"top": (DH - infodH) / 2
		}).show();
		jQuery(id).children(".supIco").hide();
	}
}
//
function loadDevice() {
...
function addRealPlantInfo(obj, index) {
	...
	if (real.floristicsType == 2) {
		...
		jQuery(".xiugeng_hasData_begin").html("落叶休眠<br>" + dz);
		jQuery(".xiugeng_hasData_center").html("采收开始<br>" + cs);
		jQuery(".xiugeng_hasData_end").html("采收结束<br>" + js);
		jQuery("#xiugeng_noData").hide();
		jQuery("#xiugeng_hasData").show();
		b = cs;
		e = js;
		b = b.split("-")[0];
		e = e.split("-")[0];
		...
		if (real.plantId == 0) {
			jQuery(".cropsData_begin").html("休耕开始<br>" + real.middle);
			jQuery(".cropsData_end").html("休耕结束<br>" + real.plantEndTime);
			jQuery("#xiugeng_noData").show();
			jQuery("#xiugeng_hasData").hide();
		} else {
			jQuery(".xiugeng_hasData_begin").html("定植时间<br>" + real.plantBeginTime);
			jQuery(".xiugeng_hasData_center").html("采收开始<br>" + real.middle);
			jQuery(".xiugeng_hasData_end").html("种植结束<br>" + real.plantEndTime);
			jQuery("#xiugeng_noData").hide();
			jQuery("#xiugeng_hasData").show();
		}
		b = real.middle;
	...
	if (real.listStr != "") {
		...
		jQuery("#chartInfo").hide();
	};
	jQuery("#xiugeng .cropsPic img").attr("src",  real.plantImgRound);
	if (obj.realPlantInfo.length == 1) { //种了1个
		jQuery("#xiugeng h2").html(real.breedName + '&nbsp;&nbsp;' + real.tunnelName);
		jQuery(".data_select").hide();
		jQuery("#realPlantInfo_select").unbind("change").html('');
	} else { //种了好几个
		jQuery("#realPlantInfo_select").unbind("change").html(''); //先解除绑定，否则改变内容也会触发onchange
		jQuery("#xiugeng h2").html(real.tunnelName);
		jQuery(".data_select").css("display", "inline-block");
		for (var j = 0; j < obj.realPlantInfo.length; j++) {
			...
			jQuery("#realPlantInfo_select").append(options);
		}
		jQuery("#realPlantInfo_select").selectpicker().selectpicker('val', index);
		jQuery("#realPlantInfo_select").bind("change", function() {
			var val = jQuery(this).val();
			addRealPlantInfo(obj, val);
		});
	};
	jQuery("#xiugeng .farmer_mess").html("农户&nbsp;&nbsp;" + real.farmerName + "&nbsp;&nbsp;" + real.cropArea + "亩/" + real.tunnelArea + "亩");
	setPopOutDivPosition("#xiugeng"); //数据装载完成后弹出指定DIV并定位
	var xiugengH2W=jQuery("#xiugeng h2").width();
	if(xiugengH2W>235){
		jQuery("#xiugeng .data_select").addClass("dsMaxwidth");
	}
	else{
		jQuery("#xiugeng .data_select").removeClass("dsMaxwidth");
	}
}
//
function addTunnelInfo(json) {
	...
	jQuery("#noCrops h2").html(h2text);
	jQuery("#noCrops .farmer_mess").html('农户&nbsp;&nbsp;' + tunnelInfo.farmerName + '&nbsp;&nbsp;' + tunnelInfo.muArea + '亩/' + tunnelInfo.tunnelArea + '亩');
	var tunnelLandTest = json.tunnelLandTest[0];
	if (json.tunnelLandTest > 0) {
		jQuery("#noCrops .jiance_mechanism").html('检测机构：' + (tunnelLandTest.testOrg == '' ? "暂无数据" : tunnelLandTest.testOrg)).attr("title", (tunnelLandTest.testOrg == '' ? "暂无数据" : tunnelLandTest.testOrg));
		jQuery("#noCrops .jiance_date").html('检测日期：' + (tunnelLandTest.testDate == '' ? "暂无数据" : tunnelLandTest.testDate));
		jQuery("#noCrops .landRecord_wp_p1").eq(0).html(tunnelLandTest.ph == "" ? "--" : tunnelLandTest.ph); //PH
		jQuery("#noCrops .landRecord_wp_p1").eq(1).html(tunnelLandTest.ec == "" ? "--" : tunnelLandTest.ec); //EC
		jQuery("#noCrops .landRecord_wp_p1").eq(2).html(tunnelLandTest.pb == "" ? "--" : tunnelLandTest.pb); //PB
	} else {
		jQuery("#noCrops .jiance_mechanism").html('检测机构：暂无数据');
		jQuery("#noCrops .jiance_date").html('检测日期：暂无数据');
		jQuery("#noCrops .landRecord_wp_p1").eq(0).html("--"); //PH
		jQuery("#noCrops .landRecord_wp_p1").eq(1).html("--"); //PH
		jQuery("#noCrops .landRecord_wp_p1").eq(2).html("--"); //PH
	}
	var tunnelSoilHistory = json.tunnelSoilHistory;
	jQuery("#noCrops .landHistory_list").html("");
	if (tunnelSoilHistory.length == 0) {
		var thisYear = new Date().getFullYear();
		jQuery("#noCrops .landHistory_list").html('<li><p class="historyP1">' + thisYear + '-' + thisYear + '</p><p class="historyP2">暂无数据</p></li>');
	} else {
		for (var i = 0; i < tunnelSoilHistory.length; i++) {
			...
			lis += '</li>';
			jQuery("#noCrops .landHistory_list").append(lis);
		}
	}
}
//
	...
	ajaxConfig({
		...
		backfun: function(json) {
			...
			if (device_type_id == 101) {
				jQuery("#sensor h2").html("奥科美二合一传感器");
			} else if (device_type_id == 102) {
				jQuery("#sensor h2").html("奥科美四合一传感器");
			} else if (device_type_id == 103) {
				jQuery("#sensor h2").html("奥科美六合一传感器");
			}
			var sensorSN = '<span>编号：' + sn + '</span>';
			jQuery("#sensor h2").append(sensorSN);
			jQuery("#sensor .sensor_c2_p1").html('露点：' + reportInfo.ludian + ' ℃ ');
			jQuery("#sensor .sensor3date p.font_my").eq(0).html('温度 ' + reportInfo.soilTemp + ' ℃');
			jQuery("#sensor .sensor3date p.font_my").eq(1).html(reportInfo.illumination + 'LUX');
			jQuery("#sensor .sensor3date p.font_my").eq(2).html(reportInfo.co2 + ' ppm');
			jQuery("#sensor .sensor_bottom").html('采集时间：' + reportInfo.genTime);
			jQuery("#sensor .sensor_c2 p").eq(1).html('湿度：' + reportInfo.airHumidity + ' %');
			jQuery("#sensor .sensor_c").html('<img src="' + assetRoot + '/asset/images/cloud.png" />' + reportInfo.airTemp + '℃');
...
function showDevice_weather(sn) {
	...
	ajaxConfig({
		...
		backfun: function(json) {
			...
			jQuery("#weatherStation h2 span").html("编号：" + (weatherStation.device_sn ? weatherStation.device_sn : ""));
			jQuery("#weatherStation .ws_data_1_c").html(parseFloat(weatherStation.air_temp) > -99 ? weatherStation.air_temp + " ℃" : "无数据");
			jQuery("#weatherStation .ws_data_1_time").html("采集时间：" + (weatherStation.gen_time ? weatherStation.gen_time : "无数据"));
			jQuery("#ws_data_1_uld1").html("降水量" + weatherStation.rainfall + "mm");
			jQuery("#ws_data_1_uld2").html(parseFloat(weatherStation.air_humidity) > -99 ? "湿度" + weatherStation.air_humidity + "%" : "");
			jQuery("#ws_data_1_uld3").html(weatherStation.wind_direction ? (weatherStation.wind_direction.flName + weatherStation.wind_direction.flClass) : "");
			var TuRang = ""; //jQuery(".ws_data_1_ul2 li").eq(0).children(".p2")
			...
			jQuery(".ws_data_1_ul2 li").eq(0).children(".p2").html(TuRang);
			jQuery(".ws_data_1_ul2 li").eq(1).children(".p2").html((parseFloat(weatherStation.illumination) > -99 ? "光照" + weatherStation.illumination + "LUX<br/>" : "") + (parseFloat(weatherStation.solar_radiation) > -99 ? "辐射" + weatherStation.solar_radiation + "w/㎡" : ""));
			jQuery(".ws_data_1_ul2 li").eq(2).children(".p2").html(parseFloat(weatherStation.atmospheric_pressure) > -99 ? weatherStation.atmospheric_pressure + "Pa" : "");
			jQuery(".ws_data_1_ul2 li").eq(3).children(".p2").html(parseFloat(weatherStation.pm25) > -99 ? weatherStation.pm25 : "");
...
function loadWeatherInfo(json) {
	...
	jQuery("#weather .w_address").html(reportInfo.district_cn + '&nbsp;&nbsp;' + reportInfo.name_cn);
	var iconTypeRoot = reportInfo.pictureTypeSn == "d/" ? "day" : "night/";
	jQuery("#weather .w_pic1 img").attr("src", assetRoot + '/asset/images/icon/' + iconTypeRoot + (parseInt(reportInfo.pictureType) <= 9 ? "0" + reportInfo.pictureType : reportInfo.pictureType) + '.png');
	jQuery("#weather .w_data1_no").html(reportInfo.airTemp);
	jQuery("#weather .w_data1_c").html(reportInfo.theWeather);
	jQuery("#weather .w_c_High").html(reportInfo.dayHigh + '°');
	jQuery("#weather .w_c_Low").html(reportInfo.dayLow + '°');
	var now = new Date();
	now.setDate(now.getDate() + 1);
	jQuery("#w_data_tomorrow").html(now.format("MM月dd日"));
	now.setDate(now.getDate() + 1);
	jQuery("#w_data_afterTomorrow").html(now.format("MM月dd日"));
	jQuery("#w_data_tomorrow_pic img").attr("src", assetRoot + '/asset/images/icon/day/' + reportInfo.day2 + '.png');
	jQuery("#w_data_afterTomorrow_pic img").attr("src", assetRoot + '/asset/images/icon/day/' + reportInfo.day3 + '.png');
	jQuery("#weather .w_Wrlist_w").html('降水量 ' + reportInfo.rainfall);
	jQuery("#weather .w_Wrlist_r").html(reportInfo.windd + reportInfo.winds + '级');
	jQuery("#w_data_tomorrow_c").html(reportInfo.day2High + '° / ' + reportInfo.day2Low + '°');
	jQuery("#w_data_afterTomorrow_c").html(reportInfo.day3High + '° / ' + reportInfo.day3Low + '°');
	jQuery("#weather").show();
	jQuery("#weather_close").click(function() {
		jQuery("#weather").slideUp("fast");
	});
...
function showKrpano_iframe(json) {
	...
	if (json.fileType == 1) { //360全景iframe
		...
		jQuery(window).bind("resize", resetFrameSize);
		...
		jQuery("#krpano_iframe").attr("src", src);
		jQuery(oimg).css({
			"position": "absolute",
			"right": "0px",
			"top": "0px",
		...
		jQuery("body").append(oimg);
		jQuery("#krpano_close_btn").unbind("click");
		jQuery("#krpano_close_btn").bind("click", function() {
			closeKrpanoFrame();
			jQuery(this).remove();
		});
		jQuery("#map_loading").hide();
		...
		farmeImg.onload = function() {
			...
			jQuery(farmeImg).delay(200).fadeIn(100, function() {
				setTimeout(function() {
					jQuery(oimg).css({
						"position": "absolute",
						"left": "50%",
		...
		jQuery("body").append(farmeImg);
	}
}
//
function closeKrpanoFrame() {
	jQuery(window).unbind("resize", resetFrameSize);
...
function resetFrameSize() {
	if (jQuery("#krpano_iframe").is(":visible")) {
		var height = jQuery(window).height();
		var width = document.body.clientWidth;
		jQuery("#krpano_iframe").css({
			"width": width,
			"height": height,
			"position": "fixed",
			"z-index": 1000,
...
function showMonitor(json) {	
	...
	if (tempThis.arr.video_webcam_url != "" && !/msie/.test(navigator.userAgent.toLowerCase())) {
		...
		window.a.init();
		//jQuery('#login3').show();
		jQuery("#video_iframe").hide();
		jQuery("#video_iframe2").hide();
		jQuery("#video_iframe3").show();
	} else {
		if (tempThis.arr.video_access_way == null || tempThis.arr.video_access_way == 1) {
			jQuery("#video_iframe2").show();
			...
			jQuery("#switch1").closest(".vb").attr("class", "videobut_ vb");
			jQuery("#switch1").unbind("click");
			jQuery("#switch1").bind("click", function() {
				jQuery(this).closest(".vb").attr("class", "videobut vb");
				openVideo();
			});
		} else if (tempThis.arr.video_access_way == 3) {
			if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
				jQuery(".videoimg__").html("");
				jQuery(".videoimg__").append('<img src="' + assetRoot + '/asset/images/zwsp.png"  height="418" />');
				jQuery("#video_iframe4").show();
			} else {
				jQuery(".videoimg__").html("<div id='divPlugin' style='width:100%;height:100%;'></div>");
				jQuery("#switch3").closest(".vb").attr("class", "videobut_ vb");
				jQuery("#video_iframe4").show();
				// 初始化插件参数及插入插件
				setTimeout(function() {
					...
					jQuery("#switch3").unbind("click");
					jQuery("#switch3").bind("click", function() {
						jQuery(this).closest(".vb").attr("class", "videobut vb");
...
function newVideoPreview(szIP, iStreamType, iChannelID, bZeroChannel) {
	...
	if (iRet == 0) {
		showOPInfo("开启阅览成功...");
		jQuery("#video_iframe4 .fdimg1").show();
		jQuery("#video_iframe4 .fdimg2").hide();
	} else {
		showOPInfo("开启阅览失败...");
		jQuery("#video_iframe4 .fdimg1").hide();
		jQuery("#video_iframe4 .fdimg2").hide();
	}
	jQuery("#switch3").unbind("click");
	jQuery("#switch3").bind("click", function() {
		jQuery(this).closest(".vb").attr("class", "videobut_ vb");
		clickStopRealPlay();
	});
}
//停止预览
...
function StartPlayView() {
	...
	try {
		...
		OCXobj.SetOcxMode(0);
		OCXobj.SetWndNum(1);
		jQuery("#video_iframe2 .fdimg1").show();
		jQuery("#video_iframe2 .fdimg2").hide();
		...
		jQuery("#video_iframe2 .fdimg1").hide();
		jQuery("#video_iframe2 .fdimg2").hide();
	}
}
...
function openVideo() {
	try {
		...
		jQuery("#video_iframe .fdimg1").show();
		jQuery("#video_iframe .fdimg2").hide();
	} catch (e) {
		jQuery("#video_iframe .fdimg1").hide();
		jQuery("#video_iframe .fdimg2").hide();
	}
}
...
function PTZControlOn(temp) {
	if (temp == 12) {
		jQuery("#v1").attr("src", assetRoot + "/asset/images/growth/onbut_jian.png");
	}
	if (temp == 11) {
		jQuery("#v2").attr("src", assetRoot + "/asset/images/growth/onbut_jia.png");
	}
	if (temp == 21) {
		jQuery("#v3").attr("src", assetRoot + "/asset/images/growth/onbut_up.png");
	}
	if (temp == 23) {
		jQuery("#v4").attr("src", assetRoot + "/asset/images/growth/onbut_left.png");
	}
	if (temp == 24) {
		jQuery("#v5").attr("src", assetRoot + "/asset/images/growth/onbut_right.png");
	}
	if (temp == 22) {
		jQuery("#v6").attr("src", assetRoot + "/asset/images/growth/onbut_down.png");
	}
	if (temp == 14) {
		jQuery("#v7").attr("src", assetRoot + "/asset/images/growth/onbut_jian.png");
	}
	if (temp == 13) {
		jQuery("#v8").attr("src", assetRoot + "/asset/images/growth/onbut_jia.png");
	}
	//channelNo:函数设备通道号
...
function PTZControlOff(temp) {
	if (temp == 12) {
		jQuery("#v1").attr("src", assetRoot + "/asset/images/growth/but_jian.png");
	}
	if (temp == 11) {
		jQuery("#v2").attr("src", assetRoot + "/asset/images/growth/but_jia.png");
	}
	if (temp == 21) {
		jQuery("#v3").attr("src", assetRoot + "/asset/images/growth/but_up.png");
	}
	if (temp == 23) {
		jQuery("#v4").attr("src", assetRoot + "/asset/images/growth/but_left.png");
	}
	if (temp == 24) {
		jQuery("#v5").attr("src", assetRoot + "/asset/images/growth/but_right.png");
	}
	if (temp == 22) {
		jQuery("#v6").attr("src", assetRoot + "/asset/images/growth/but_down.png");
	}
	if (temp == 14) {
		jQuery("#v7").attr("src", assetRoot + "/asset/images/growth/but_jian.png");
	}
	if (temp == 13) {
		jQuery("#v8").attr("src", assetRoot + "/asset/images/growth/but_jia.png");
	}
