...
var uploader1 = new plupload.Uploader({
    ...
    filters: {
        ...
        { title : "Image files", extensions : "mp4,avi,rmvb,wmv,3gp,amv,dmv,rm,mov" }, 
        ],
	...
	init: {
		...
		FilesAdded: function(up, files) {
			plupload.each(files, function(file) {
				document.getElementById('ossfile').innerHTML = '<div style="width:50%; margin-left:25%;"  id="' + file.id + '"><b1></b1>'
		...
		FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
            	...
            	document.getElementById("aerialFileUrl").value = uploaded_file_url;
            	document.getElementById("aerialFileName").value = file.name;
		...
		Error: function(up, err) {
            if (err.code == -600) {
            	var filters = up.getOption("filter");
            	alert("上传视频大于200M!");
            	...
            	alert("视频格式不正确,请上传"+extensions+"格式的图片");
            	...
            	alert("暂时不支持当前浏览器，请更换火狐、谷歌浏览器或者将360浏览器切换为极速模式");
            }
		}
	}
});
