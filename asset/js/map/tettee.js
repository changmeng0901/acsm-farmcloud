...
Element.Methods = {
	...
	viewportOffset : function(forElement) {
		...
		} while ((element = element.parentNode) && element.tagName);
		return Element._returnOffset(valueL, valueT);
	},
...
if (!window.RichFaces.Memory) {
			...
			if (oldNode) {
				...
				if (all) {
					...
					for ( var counter = 0; counter < length; counter++) {
						this
								.applyCleaners(all[counter], isAjax,
										componentNodes);
					}
				} else {
	...
	window.RichFaces.Memory.addComponentCleaner("richfaces", function(node,
			isAjax) {
...
Element.isUninitialized = function(element) {
	if (element) {
		if (element.nodeType == Node.ELEMENT_NODE) {
			if (!element.parentNode || element.document
					&& element.document.readyState == "uninitialized") {
				return true;
			} else {
...
if (window.RichFaces && window.RichFaces.Memory) {
	window.RichFaces.Memory.addCleaner("prototype", function(node, isAjax) {
		var eventID = node._prototypeEventID ? node._prototypeEventID[0]
				: undefined;
		if (eventID) {
