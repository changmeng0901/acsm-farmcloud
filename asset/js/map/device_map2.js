...
var drawingManager;
var deviceTypeObj;
var devicesrc = workpath+"/asset/images/vidicon3.png";
...
//			alert("加载大棚");
	...
	function loadfifth(){
		for(var i = 0;i<videoStr.length;i++){
			var temp = videoStr[i];
			var arr = {};
			if(temp.coordinateX != null){
				device_coordinateX	= temp.coordinateX;
				device_coordinateY	= temp.coordinateY;
			}else{
				arr.is_drag = true;
			}
			arr.device_ = 3;
			...
			arr.video_device_host = temp.deviceHost;
			arr.video_device_port = temp.devicePort;
			arr.video_device_username = temp.deviceUsername;
			arr.video_device_password = temp.devicePassword;
			arr.video_restart = temp.restart;
...
//				qxzObj.setMap(null);
	...
	function save_ok(){
		isExist = true;
		 if(arr.device_ == 3){
		     _Q("#user\\:device_coordinateX").val(device_coordinateX);
			 _Q("#user\\:device_coordinateY").val(device_coordinateY);
		}
		_Q("#user\\:partition_id").val(parId);
		_Q("#user\\:tunnel_id").val(tunnelId);
		drawingManager.setDrawingMode(null);
    ...
    CustomOverlay.prototype.setNewPoint = function(){
    	...
    	if(this.arr.device_ == 3){
		     _Q("#user\\:device_coordinateX").val(this.quadrilateral.oldlatLng.lat());
			 _Q("#user\\:device_coordinateY").val(this.quadrilateral.oldlatLng.lng());
		}
    	this.draw();
    }
