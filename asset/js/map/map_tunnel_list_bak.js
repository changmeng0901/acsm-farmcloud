		...
		function addPlant(){
			...
			_$("#plant_area").val(allarea.toFixed(2));
		...
		function getModelByTime_next(data){
			...
			if(_$("#edittype").val()==1){
				...
				var date = new Date(_$("#plant\\:plant_beginInputDate").val());
				var fl = 0;
				var start = new Date(_$("#old_start").val());
				var end = new Date(_$("#old_end").val());
				if(date<start||date>end){
					fl = parseFloat(strs[0]);
				}else{
					fl = parseFloat(strs[0])+parseFloat(_$("#myarea").val()); 
				}
				_$("#plantarea").html(fl.toFixed(2));
