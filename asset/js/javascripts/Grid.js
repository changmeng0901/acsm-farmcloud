...
(function(window, document, undefined) {
	...
	GridProto.generateSkeleton = function() {
		...
		for (var i=0, elem; elem=elems[i]; i++) {
			(this[elem[0]] = doc.createElement("DIV")).className = elem[1]+""+(typeof(elem[3])=='undefined'?"":" "+elem[3]+""+this.options.viewId);
	...
	GridProto.convertData = function(data) {
		...
		if ((base = data.Head || data.Body || data.Foot || null)) {
			...
			if (data.Head) {
				...
				this.css.rules[".g_Head"+this.options.viewId] = { display : "none" };
			}
			if (data.Body) {
	...
	GridProto.generateGridSection = function(cols) {
		...
		while (colIdx) {
			if ((--colIdx) < fixedCols) {
				fHtml[colIdx] = "<DIV class='g_Cl g_Cl" + this.options.viewId + colIdx + " g_FCl'>@" + colIdx + "@</DIV></DIV>";
				sHtml[colIdx] = "<DIV class='g_Cl g_Cl" + this.options.viewId + colIdx + " g_FCl'></DIV>";
			} else {
				sHtml[colIdx] = "<DIV class='g_Cl g_Cl" + this.options.viewId + colIdx + "'>@" + colIdx + "@</DIV></DIV>";
			}
		}
	...
	GridProto.alignColumns = function(reAlign, fromInit) {
		...
		if (reAlign !== true) {
			...
			for (var i=0, len=this.columns; i<len; i++) {
				rules[".g_Cl" + this.options.viewId + i].width = "auto";
			}
			this.setRules();
		}
		...
		for (var i=0, len=this.columns; i<len; i++) {
			...
			if(this.options.colWidths.length>i){
				colWidth = this.options.colWidths[i];
			}else{
				colWidth = Math.max((nodes[0][i] || {}).offsetWidth || 90, 
	                    (nodes[1][i] || {}).offsetWidth || 90, 
	                    (nodes[2][i] || {}).offsetWidth || 90);
			}
			
			
			this.columnWidths[i] = colWidth;
			rules[".g_Cl" + this.options.viewId + i] = { "width" : colWidth + "px", "text-align" : (colAlign[i] || "center") };
			if ((colBGColors[i] || "#ffffff") !== "#ffffff") {
				rules[".g_Cl" + this.options.viewId + i]["background-color"] = colBGColors[i];
			}
	...
	GridProto.computeBaseStyles = function() {
		...
		if (this.hasHead) {
			rules[".g_Head"+this.options.viewId] = { "right" : sBarSize.x + "px" };
		}
		if (this.hasFoot) {
			rules[".g_Foot"] = { "bottom" : sBarSize.y + "px", "right" : sBarSize.x + "px" };
		}
		if (this.hasFixedCols) {
			rules[".g_BodyFixed" + ((msie < 8) ? "2" : "")+this.options.viewId] = { "top" : headHeight + "px", "bottom" : sBarSize.y + "px" };
		}
		if (this.options.allowGridResize) {
	...
	GridProto.delegateHeaderEvent = function(event) {
		...
		if (event.button !== 2) {
			if (this.options.allowColumnResize && targetClass.indexOf("g_RS") > -1) {
				...
				while (targetClass.indexOf("g_Cl") === -1 && targetClass.indexOf("g_Head")==-1) {
	...
	GridProto.endResizeColumn = function(event) {
		...
		this.css.rules[".g_Cl" + this.options.viewId + colIdx]["width"] = newWidth + "px";
	...
	GridProto.selectRange = function(event) {
		...
		if (event.button !== 2 && this.options.allowSelections) {
			targetClass = target.className || "";
			while (targetClass.indexOf("g_BR") === -1 && targetClass.indexOf("g_Body")==-1) {
				targetClass = (target = target.parentNode).className || "";
			}
