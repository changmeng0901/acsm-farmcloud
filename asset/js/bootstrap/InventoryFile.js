...
$(function(){
	...
	$('.collapse_btn').click(function(){
		if( menuOnOff == true ){
			$('.inbentory_level').hide();
			$('.inbentory_content').addClass('hide_inben');	
			$('.inbentory_content').outerWidth( $('body').width()-77-0 );
			menuOnOff = false;
		}else{
			$('.inbentory_level').show();
			$('.inbentory_content').removeClass('hide_inben');	
			$('.inbentory_content').outerWidth( $('body').width()-77-$('.inbentory_level_wap').width() );
			menuOnOff = true;		
		}	
	});
	...
	$('.inbentory_level').css({ 
		'min-height' : oWindowH - 60 -IESpace,
		    //'height' : $(document).height() - 60  -IESpace
			 'height' : $('.inbentory_content').outerHeight()  -IESpace
	});
	//$('.inbentory_level').niceScroll({cursorcolor:"#919191",cursorwidth:10,cursoropacitymax:0.7,touchbehavior:false,autohidemode:false}); 
...
//		$(this).parents().find('.mater_name_bd').show();
	...
	$('body').click(function(){
		$('.mater_name_bd').hide();	
		$('.mater_name_bd2').hide();	
		$('.mater_name_list2').each(function(){
			$(this).empty();
		});
	});
