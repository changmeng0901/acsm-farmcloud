...
// (1)查看更多功能
function ViewMorebaseFn(obj,oDownName,oMoreCon){
	//默认是关闭的
...
function ViewMoreFn(obj,oDownName,oMoreCon){
	...
	if($(obj).hasClass(oDownName)){
		//如果点击时，是展开的图标，则收起更多内容及图标变为收起图标
		$(obj).removeClass(oDownName);
		//$(oMoreCon).css({'height':'0','overflow':'hidden'});
		$(oMoreCon).attr('style','height:0;overflow:hidden;display:block;');
	}else{
		$(obj).addClass(oDownName);
		//$(oMoreCon).css({'height':'auto','overflow':'auto'});
		$(oMoreCon).attr('style','height:auto;display:block;');
	}
}
...
$('.btn_screen').click(function(){
	...
	if($(this).attr('onoff') == 'false'){
		...
		$(this).find('img').attr('src','../../../../asset/images/digitalCard/card/btn_smallscreen.png');
		$('#big_map').addClass('bigmap');
		$('.card_main').css('overflow-y','hidden');
		$('.markmapopa').hide();
		$("#digitalCardMapFrame")[0].contentWindow.displayZoomContral(true);
	}else{
		$(this).attr('onoff','false');
		$(this).find('img').attr('src','../../../../asset/images/digitalCard/card/btn_fullscreen.png');
		$('#big_map').removeClass('bigmap');
		$('.card_main').css('overflow-y','auto');
		$('.markmapopa').show();
		$("#digitalCardMapFrame")[0].contentWindow.displayZoomContral(false);
	}
});
